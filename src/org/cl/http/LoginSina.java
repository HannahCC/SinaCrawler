...
public class LoginSina
{
	...
	private String getnonce()
	{
		...
		for (int i = 0; i < 6; i++)
		{
			str += x.charAt((int) Math.ceil(Math.random() * 1000000)
					% x.length());
		}
		return str;
	}
	...
	private void init()
	{
		...
		PreLoginResponseMessage plrmsg = JsonUtils
				.jsontoPreLoginResponseMessage(responseText);
	...
	private void downloadCheckImage()
	{
		...
		String cookieValue = HttpUtils.setCookie2String(this.cookies);
		this.headers.put("Cookie", cookieValue);
		String url = "http://login.sina.com.cn/cgi/pin.php?r="
				+ (long) (Math.random() * 100000000) + "&s=0&p=" + this.pcid;
		System.out.println(url);
		HttpResponse response = HttpUtils.doGet(url, headers);
		InputStream in = HttpUtils.getInputStreamFromResponse(response);
		SaveInfo.writeFileFromStream("image/checkImage.jpeg", in);
	}
	...
	private String compositeUrl()
	{
		...
		builder.append("http://login.sina.com.cn/sso/prelogin.php?").append(
				"entry=weibo&callback=sinaSSOController.preloginCallBack&")
				.append("su=" + getEncodedU()).append(
						"&rsakt=mod&checkpin=1&client=ssologin.js(v1.4.5)&_="
	...
	private String getEncodedU()
	{
		...
		{
			return Base64Encoder.encode(EncodeUtils
					.encodeURL(username, "utf-8").getBytes());
		}
		return "";
	}
	...
	public static String rsaCrypt(String pubkey, String exponentHex,
			String pwd, String servertime, String nonce)
	{
		String data = servertime + "\t" + nonce + "\n" + pwd;
	...
	/**
	 *e.rsakv = a.rsakv; var f = new sinaSSOEncoder.RSAKey;
	 * f.setPublic(a.rsaPubkey, "10001"); c = f.encrypt([a.servertime,
	 * a.nonce].join("\t") + "\n" + c)
	 */
	public static String rsaCrypt(String pubkey, String exponentHex,
			String messageg)
	{
		...
		BigInteger publicExponent = new BigInteger(pubkey, 16); /*
																 * public
																 * exponent
																 */
